...
import { ACCOUNT_TYPE_LIST } from "../../../constants/Query";
...
import { DELETE_ACCOUNT_TYPE } from "../../../constants/Mutation";
import { nameTypeSearch } from "../../../propTypes/propTypes";
import settings from "../../../theme/settings";

const theader = ["Name", "Status", "Actions"];
const tbodyFields = ["name", "status", "action"];
...
const AccountType: React.FC = () => {
  ...
  const [accountTypeList, { loading, data, error }] = useLazyQuery(ACCOUNT_TYPE_LIST);
  const [removeAccountType, { loading: r_loading, error: r_error, data: r_data }] =
    useMutation(DELETE_ACCOUNT_TYPE);
  ...
  useEffect(() => {
    if (!loading && data && !error) {
      const { response, total } = data.accountTypeList;
      setRecords(() => ({
  ...
  useEffect(() => {
    if (!r_loading && !r_error && r_data) {
      const response = r_data.removeAccountType;
  ...
  return (
    <Content>
      <SubHeaderAction
        ...
        addNewLink="/admin/account-type/create"
