...
import Form from "../../organisms/CardType/Form";
import { nameStatusInputTypes } from "../../../propTypes/propTypes";
import { FIND_CARD_TYPE_BY_ID } from "../../../constants/Query";
import { EDIT_CARD_TYPE } from "../../../constants/Mutation";
...
const UpdateCardType: React.FC = () => {
  ...
  const [updateCardType, { loading, error, data }] =
    useMutation(EDIT_CARD_TYPE);
  const [
    findCardTypeById,
    { loading: f_loading, error: f_error, data: f_data },
  ] = useLazyQuery(FIND_CARD_TYPE_BY_ID);
  const {
    register,
    handleSubmit,
    setError,
  ...
  // Loads default data
  useEffect(() => {
    findCardTypeById({ variables: { input: { _id: id } } });
  }, [id, findCardTypeById]);

  // Response: Loads data
  useEffect(() => {
    if (!f_loading && !f_error && f_data) {
      const response = f_data.findCardTypeById;
  ...
  useEffect(() => {
    if (!loading && !error && data) {
      const response = data.updateCardType;
