...
import { CREATE_CARD_TYPE } from "../../../constants/Mutation";
...
const CreateCardType: React.FC = () => {
  const navigate = useNavigate();
  const [, dispatch] = useStateValue();
  const [severity, setSeverity] = useState("error");
  const [message, setMessage] = useState<any>("");
  const [createCardType, { loading, error, data }] =
    useMutation(CREATE_CARD_TYPE);
  const {
    register,
    handleSubmit,
    setError,
  ...
  const onSubmit = (input: nameStatusInputTypes) => {
    createCardType({variables: {input}});
  };

  // Response: Submit Handler
  useEffect(() => {
    if (!loading && !error && data) {
      const response = data.createCardType;
