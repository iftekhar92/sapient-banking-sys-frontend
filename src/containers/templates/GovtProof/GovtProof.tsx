...
import { PROOF_TYPE_LIST } from "../../../constants/Query";
...
import { DELETE_PROOF_TYPE } from "../../../constants/Mutation";
...
const GovtProof: React.FC = () => {
  ...
  const [proofTypeList, { loading, data, error }] =
    useLazyQuery(PROOF_TYPE_LIST);
  const [
    removeProofType,
    { loading: r_loading, error: r_error, data: r_data },
  ] = useMutation(DELETE_PROOF_TYPE);
  ...
  useEffect(() => {
    if (!loading && data && !error) {
      const { response, total } = data.proofTypeList;
      setRecords(() => ({
  ...
  useEffect(() => {
    if (!r_loading && !r_error && r_data) {
      const response = r_data.removeProofType;
  ...
  return (
    <Content>
      <SubHeaderAction
        ...
        addNewLink="/admin/govt-proof/create"
